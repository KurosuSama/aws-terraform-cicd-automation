...
| **Inventory Management** | **Static/Generated File.** Relied on a static `inventory.ini` generated by Terraform. Prone to being outdated if ASG scales during deployment. | **Dynamic Inventory.** Ansible queries the AWS API directly via `aws_ec2.yml` plugin to fetch the real-time list of running instances based on tags. |
| **Security Access** | **Static/Manual.** Required manually adding the developer's IP to the Security Group or keeping port 22 open to a broad range. | **Ephemeral.** The pipeline automatically detects the Runner's IP, authorizes it in the Security Group only for the duration of the deployment, and revokes it immediately after. |
| **Environment** | **Unreliable.** Different software versions on developers' laptops often cause unexpected errors. | **Consistent.** Every deployment starts on a clean system with the exact same settings every time. |
...
* `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`: IAM credentials with permissions for EC2 and S3.
* `SSH_PRIVATE_KEY`: The private key corresponding to the Public Key uploaded to EC2 instances.
...
CI/CD Pipeline Execution (GitHub Actions) The application deployment is fully automated and triggered by changes to the main branch:

1. **Environment Setup:** Initializes the runner and installs Ansible/AWS dependencies.
2. **Network Authorization:** The runner detects its own public IP and temporarily adds an allow rule to the AWS Security Group.
3. **Dynamic Discovery:** Ansible queries AWS to find all currently running instances in the Auto Scaling Group.
4. **Configuration:** The playbook deploys the Dockerized application to all discovered nodes.
5. **Security Cleanup:** The pipeline removes the temporary IP rule from the Security Group, restoring strict access control.
